 ...
 *
 * They are ordinary Vue stories rather than components written inline in the MDX, so that
 * everything in this repo stays Vue. The MDX pages carry the prose and pull each specimen in with
 * `<Story of={...} />`. `!dev` keeps them out of the sidebar — they are only meaningful inside the
...
export const HeadingScale: TypeStory = {
	render: (args) => ({
		...
		setup: () => ({ args }),
		template: '<TypeScale v-bind="args" />',
	}),
	args: {
		token: '--font-heading',
		samples: [
