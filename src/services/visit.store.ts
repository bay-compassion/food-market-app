 ...
 * refresh that keeps queue position current while it's still live. Scoped to `GuestView`'s mount —
 * constructed fresh each time it mounts, disposed when it unmounts — since `/signup` never has a
 * visit to show.
 */
export class VisitStore {
	...
	/** Looks up the stored token's visit, if any, and schedules the next refresh. */
