...
describe('AddGuestSection', () => {
	...
	it('lets a worker choose the draw or a reserved spot before the lottery runs', async () => {
		const { container } = await openForm(['lottery', 'queue']);
		// [0] is the age range select, which is always present.
		const options = optionTexts(selects(container)[1]!);
	...
	it('drops the choice when the session only allows one way in', async () => {
		...
		expect(container.textContent).toContain(t.admitToQueueHelp);
	});

	it('hides the queue placement for an admission that never joins a line', async () => {
		const { container } = await openForm(['served']);

		// Only the age range select remains.
		expect(selects(container)).toHaveLength(1);
		expect(container.textContent).toContain(t.admitAsServedHelp);
	});

	it('offers the draw odds only to a guest actually entering the draw', async () => {
		const { container, user } = await openForm(['lottery', 'queue']);

		expect(container.textContent).toContain(t.lotteryWeightLabel);
		// Switching to a reserved spot takes the guest out of the draw, so the odds go with it.
		await user.selectOptions(selects(container)[1]!, 'queue');
		expect(container.textContent).not.toContain(t.lotteryWeightLabel);
	});
	...
	it('starts a weighted guest on even odds', async () => {
		const { container, onAddGuest } = await openForm(['lottery']);
	...
	it('emits the guest with the admission the worker picked', async () => {
		const { container, onAddGuest, user } = await openForm(['lottery', 'queue']);
