...
import { pushSubscriptions, smsOptOuts, smsSubscriptions, visits } from '../../../db/schema.mjs';
...
notificationStatusRoutes.get('/api/notification-status', withDeviceGuest, async (context) => {
	...
	const [[pushSubscription], [smsSubscription], [smsOptOut]] = await Promise.all([
		db
	...
	return Response.json({
		...
		smsOptOutSender: smsOptOut?.senderPhone ?? null,
	});
});
