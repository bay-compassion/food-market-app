...
- An MDX docs page joins the review document by tagging itself: `<Meta title="Guest/Forms" tags={['review']} />` for a numbered section, `tags={['review-front']}` for the title page. Its place in the document is its place in the sidebar, which is the `Guest` list in the preview's `storySort`. Every word in the document is written in MDX; the stories on a page are its figures, and `<Still id="…" />` embeds a photograph of the running app for a screen a story cannot show (a dialog, a failure, a form just submitted). `npm run capture:stills` takes the photographs and prints the pages (see `docs/stills.md`).
