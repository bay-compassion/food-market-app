...
- Every story is wrapped in a fresh `RootStore` and a memory router, so a component can call `useRootStore()`, `useTranslation()`, or `useNavigate()` without the story arranging anything. The store is built once per story, and the toolbar's locale picker sets its language — which is what re-renders the shell with `dir="rtl"` for Arabic and Farsi. A story that needs seeded state (a submitting form, a failed request) nests its own `RootStoreProvider` inside, which wins. The decorator also mounts `ConfirmationDrawer`, as `App` does, so a component that awaits `confirmation.ask()` shows its sheet in a story without arranging one — a `play` function reaches it through `within(document.body).findByRole('alertdialog')`, since it renders in a portal. A story that nests its own provider must nest a `ConfirmationDrawer` too (see `GuestVisitStatus.stories.tsx`): the decorator's sheet reads the decorator's store, so against a seeded one it never opens and the `ask()` is left waiting. Match text in a `play` function against the entry in `locales.ts` rather than a literal string.
