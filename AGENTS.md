...
- Storybook configuration lives in `.storybook/`. Stories sit next to the component they cover (`src/components/AppButton.stories.tsx`); the design system documentation pages are Storybook-only content and live in `.storybook/docs/` rather than in `src/`.
...
- Every story is wrapped in a fresh `RootStore` and a memory router, so a component can call `useRootStore()`, `useTranslation()`, or `useNavigate()` without the story arranging anything. The store is built once per story, and the toolbar's locale picker sets its language — which is what re-renders the shell with `dir="rtl"` for Arabic and Farsi. A story that needs seeded state (a submitting form, a failed request) nests its own `RootStoreProvider` inside, which wins. The decorator also mounts `ConfirmationDrawer` and `NotificationToasts`, as `App` does, so a component that awaits `confirmation.ask()` or raises `notifications.error()` shows its sheet or toast in a story without arranging one — a `play` function reaches it through `within(document.body).findByRole('alertdialog')`, since it renders in a portal. A story that nests its own provider must nest a `ConfirmationDrawer` (and `NotificationToasts`, if it raises one) too (see `GuestVisitStatus.stories.tsx`): the decorator's copies read the decorator's store, so against a seeded one the sheet never opens and the `ask()` is left waiting. Match text in a `play` function against the entry in `locales.ts` rather than a literal string.
- **Stories share a window on a docs page.** An MDX page renders every story it embeds in one document, so a story that changes something global — `window.fetch`, `localStorage`, `document` — changes it for all of them, and whichever rendered last wins. A visit store refreshes itself every fifteen seconds, so a fake `fetch` made every card on a docs page turn into the last story's status. Hand the fake to the store instead: `RootStore` takes per-store options (`visit: { lookupCurrentVisit, cancelVisit, storage }`), which is what `GuestVisitStatus.stories.tsx` does. Build a store once per set of args with `useState`, keyed on the args, rather than in the render body.
- The toolbar's **App frame** global (`appFrame`) puts a `shell: 'guest'` story inside the real app bar and footer, in a phone-wide column. It is off by default; the review document turns it on for every story on its pages, so a story never has to say so.
- An MDX docs page joins the review document by tagging itself: `<Meta title="Guest/Forms" tags={['review']} />` for a numbered section, `tags={['review-front']}` for the title page. Its place in the document is its place in the sidebar, which is the `Guest` list in the preview's `storySort`. Every word in the document is written in MDX; the stories on a page are its figures, and `<Screenshot id="…" />` embeds a screenshot of the running app for a screen a story cannot show (a dialog, a failure, a form just submitted). `npm run capture:screenshots` takes the screenshots and prints the pages (see `docs/screenshots.md`).
- A store is constructed by the preview's decorator, which wraps a story's own decorators — so anything a store reads in its constructor (`localStorage` device and visit tokens) has to be seeded at module scope in the story file, not inside a decorator.
...
- The color, radius, and type specimens are components (`.storybook/docs/TokenTable.tsx`, `TypeScale.tsx`) exposed as stories in `DesignTokens.stories.tsx` and pulled into the MDX prose with `<Story of={...} />`. Those stories are tagged `!dev` so they stay out of the sidebar while still being covered by `npm run test:storybook`. Add a specimen the same way rather than writing JSX inline in an MDX file.
...
- The app is React throughout — components, routing (`react-router`), and auth (`@auth0/auth0-react`).
  There is no Vue left; anything in the history that mentions `.vue` files, `fromMobx`, or
  `src/react-bridge/` describes a codebase that no longer exists.
- **Stores are MobX observables, so a component that reads one must be wrapped in `observer()`**
  from `mobx-react-lite`. Without it the component renders once against the store and then goes
  stale — silently, with no error.
- `useRootStore()` (`src/stores/react/store-context.tsx`) is how a component reaches the store, and
  `useTranslation()` is the thin hook over it for guest-facing copy. `useRootStore()` throws when
  no provider is present rather than falling back to a fresh store: a per-call fallback hands each
...
- Transient outcomes — an action that failed or finished — are toasts: call
  `rootStore.notifications.error()` / `.success()` from a component or store, never notistack
  directly (its `enqueueSnackbar` is undefined until the provider mounts, which breaks store tests).
  Anything the person needs in order to fix a form stays inline, next to the field.
...
- Within `src/`: shared components sit directly in `src/components/`, and components belonging to one feature go in a subfolder named for it (for example `src/components/admin/`). Model classes — domain classes built from state, such as `SessionTimeline` and `RecurrencePattern` — go in `src/models/`; other business logic, API clients, state-machine lookup maps, and utilities go in `src/services/`. Existing classes in `src/services/` move to `src/models/` when you are already reworking them, not as a drive-by. CSS shared across components, rather than owned by one of them, belongs in `src/styles/` and is imported once from `src/main.tsx`.
- **`src/services/` and `src/models/` are not frontend-only.** Files under `netlify/`, plus `db/schema.mts`, import from both — they are the shared layer between browser and server. Anything placed there must therefore stay free of MobX, React, and DOM APIs, which is the practical reason domain classes are plain TypeScript rather than stores. Server code imports them with a real runtime extension (`../../src/services/reports.js`, `../../src/models/session-timeline.js`), as NodeNext resolution requires. A model the guest's first screen reads must also stay free of heavy dependencies: `SessionTimeline` uses plain `Date` arithmetic, while `MarketLocation` and `RecurrencePattern` use Luxon and are imported only by the server and lazily loaded admin code.
