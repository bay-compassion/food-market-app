...
- An MDX docs page joins the review document by tagging itself: `<Meta title="Guest/Forms" tags={['review']} />`. `npm run capture:stills` prints every page carrying the tag (see `docs/stills.md`), so the page's prose is the document's prose, and the stories embedded in it are its figures.
